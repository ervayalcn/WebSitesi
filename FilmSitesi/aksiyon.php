...
?>
    ...
    <h4 class="girisbaslik bg-dark"></h4>
        <h2 class="yazibox">Aksiyon Filmleri</h2>
        <div class="box2">
...
<div id="main2">
  <button class="openbtn" onclick="openBar2()">☰ Kategoriler</button>  
     ...
     <div>
                ...
                <?php
             $sql9 = "Select film_ID, film_adi, film_resim from filmler WHERE tur_ID=83";
    $return9 = mysqli_query($conn, $sql9);
?>
<div class="blt-row-filmler">
        <div class="row">
            <?php

      if(mysqli_num_rows($return9) > 0)
      {

          while($row9 = mysqli_fetch_assoc($return9))
          {
              echo "<div style='margin-left: 30px;margin-top: 35px' class='col-md-4 col-sm-4'>";
              echo "<a href='film.php?Fid=".$row9["film_ID"]."'><img src='" . $row9["film_resim"] . "' class='anaresim' ></a>";
              echo "<p style='color: #c9c9c9; font-size:19px;font-weight: bold; margin-left:20px'>".$row9['film_adi']."</p>";
              echo "</div>";
          }
      }
             ?>
