...
?>
    ...
    <h4 class="girisbaslik bg-dark"></h4>
        <h2 class="yazibox">Dram Filmleri</h2>
        <div class="box2">
...
<div id="main2">
  <button class="openbtn" onclick="openBar2()">☰ Kategoriler</button>  
          ...
          <div>
             ...
             <?php
             $sql8 = "Select film_ID, film_adi, film_resim from filmler WHERE tur_ID=80";
    $return8 = mysqli_query($conn, $sql8);
?>
<div class="blt-row-filmler">
        <div class="row">
            <?php

      if(mysqli_num_rows($return8) > 0)
      {

          while($row8 = mysqli_fetch_assoc($return8))
          {
              echo "<div style='margin-left: 30px;margin-top: 35px' class='col-md-4 col-sm-4'>";
              echo "<a href='film.php?Fid=".$row8["film_ID"]."'><img src='" . $row8["film_resim"] . "' class='anaresim' ></a>";
              echo "<p style='color: #c9c9c9; font-size:19px;font-weight: bold; margin-left:20px'>".$row8['film_adi']."</p>";
              echo "</div>";
          }
      }
             ?>
