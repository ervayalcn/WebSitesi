...
?>
    ...
    <h4 class="girisbaslik bg-dark"></h4>
        <h2 class="yazibox">Türkçe Dublajlı Filmler</h2>
        <div class="box2">
...
<div id="main2">
  <button class="openbtn" onclick="openBar2()">☰ Kategoriler</button>  
     ...
     <div>
         <table class="ortala">
            <?php
             $sql6 = "Select film_ID, film_adi, film_resim from filmler WHERE kategori_ID=92";
    $return6 = mysqli_query($conn, $sql6);
?>
<div class="blt-row-filmler">
        <div class="row">
            <?php

      if(mysqli_num_rows($return6) > 0)
      {

          while($row6 = mysqli_fetch_assoc($return6))
          {
              echo "<div style='margin-left: 30px;margin-top: 35px' class='col-md-4 col-sm-4'>";
              echo "<a href='film.php?Fid=".$row6["film_ID"]."'><img src='" . $row6["film_resim"] . "' class='anaresim' ></a>";
              echo "<p style='color: #c9c9c9; font-size:19px;font-weight: bold; margin-left:20px'>".$row6['film_adi']."</p>";
              echo "</div>";
          }
      }
             ?>
