...
?>
    ...
    <h4 class="girisbaslik bg-dark"></h4>
        <h2 class="yazibox">Bilim-Kurgu Filmleri</h2>
        <div class="box2">
  ...
  <button class="openbtn" onclick="openBar2()">☰ Kategoriler</button>  
     ...
     <div>
         <table class="ortala1">
                 ...
                 <?php
             $sql11 = "Select film_ID, film_adi, film_resim from filmler WHERE tur_ID=89";
    $return11 = mysqli_query($conn, $sql11);
?>
<div class="blt-row-filmler">
        <div class="row">
            <?php

      if(mysqli_num_rows($return11) > 0)
      {

          while($row11 = mysqli_fetch_assoc($return11))
          {
             echo "<div style='margin-left: 30px;margin-top: 35px' class='col-md-4 col-sm-4'>";
              echo "<a href='film.php?Fid=".$row11["film_ID"]."'><img src='" . $row11["film_resim"] . "' class='anaresim' ></a>";
              echo "<p style='color: #c9c9c9; font-size:19px;font-weight: bold; margin-left:20px'>".$row11['film_adi']."</p>";
              echo "</div>";
          }
      }
             ?>
